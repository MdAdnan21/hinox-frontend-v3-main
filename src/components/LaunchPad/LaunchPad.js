...
import LaunchPadSkelenton from "../Skelenton/LaunchPadSkelenton/LaunchPadSkelenton";
...
const LaunchPad = () => {
  ...
  const [maxWL, setMaxWL] = React.useState(0);
  ...
  const getUserPurchase = async () => {
    ...
    try {
      ...
      let wl = whitelistCap;
      setWlCap(ethers.utils.formatEther(whitelistCap));
      let total =
  ...
  const getWLcount = async () => {
    try {
      ...
      const maxWl = await ido.whitelistLength();
      setMaxWL(parseInt(maxWl));
    } catch (e) { }
  };
  ...
  return (
    <>
      {loading && <LaunchPadSkelenton />}
      {!loading && (
        <div className="container">
          <div className={styles.launchPad}>
            <div className={styles.wrapper}>
              <div className={styles.aboutProject}>
                <AboutProject details={details} isLive={isLive} />
              </div>
